...
def main() -> int:
    dataset = sys.argv[1] if len(sys.argv) > 1 else "combination"
    ...
    # Scripts in the same folder as runall.py
    gray_script = PROJECT_ROOT / "Gray1.py"
    change_gray_script = PROJECT_ROOT / "changeGray1.py"

    if not gray_script.exists():
        raise FileNotFoundError("gray.py not found: {0}".format(gray_script))
    if not change_gray_script.exists():
        raise FileNotFoundError("changeGray.py not found: {0}".format(change_gray_script))
    ...
    # The image used for picking gray id
    gray_image_path = PROJECT_ROOT / ("data/{0}/object_mask/IMG_6964.png".format(dataset))
    if not gray_image_path.exists():
        raise FileNotFoundError("Gray image not found: {0}".format(gray_image_path))

    # ----------------------------------------------------------------------
    # Step A: Mouse-pick gray id (used for removal + inpaint + changeGray)
    # ----------------------------------------------------------------------
    ...
    print("[INFO] Using target_gray = {0} for the whole pipeline".format(target_gray))

    # Write both configs automatically
    ...
    # ----------------------------------------------------------------------
    # Step 2.5: changeGray BEFORE Step3 (auto)
    # Generates: data/<dataset>/inpaint_object_mask_255/*
    # ----------------------------------------------------------------------
