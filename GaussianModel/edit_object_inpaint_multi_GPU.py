...
from utils.loss_utils import masked_l1_loss, laplacian_smoothing_loss
...
def finetune_inpaint_worker(rank, world_size, args, model_cfg, opt_cfg, pipeline_cfg):
    ...
    for local_iter in range(iters_per_proc):
        ...
        lpips_loss = LPIPS_loss_fn(rendering_patches.squeeze().to(device) * 2 - 1,
                                   gt_patches.squeeze().to(device) * 2 - 1).mean()

        # 计算 2.2.2 节的拉普拉斯平滑损失 L_smooth
        # gaussians._xyz 是当前场景中所有高斯点的三维中心坐标 \mu
        loss_smooth = laplacian_smoothing_loss(gaussians._xyz, k=5, sample_size=4000)

        # 论文 2.2.3 节的优化目标：L_bg = L_mv + \gamma * L_smooth
        # 根据论文，超参数 \gamma 在本实验中取值为 0.05
        gamma = 0.05

        # 合成最终损失 (Ll1 充当了颜色监督的 L_mv 基础)
        loss = (1.0 - opt_cfg.lambda_dssim) * Ll1 + opt_cfg.lambda_dssim * lpips_loss + gamma * loss_smooth
        loss.backward()
        # ======================================
        # LPIPS expects inputs in [-1,1]
