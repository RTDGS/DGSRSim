...
from .gr00t import n1_7_serialization
...
class Gr00tN17ServicePolicyClient(ZMQServicePolicy):
    """
    Service policy client for GR00T N1.7: https://github.com/NVIDIA/Isaac-GR00T
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            ping_endpoint="ping",
            serializer=n1_7_serialization.MsgSerializer,
        )
        self.camera_keys = camera_keys
        self.modality_keys = modality_keys

    @staticmethod
    def _add_batch_time_axes(array: np.ndarray, sample_ndim: int) -> np.ndarray:
        if array.ndim == sample_ndim:
            return array[None, None, ...]
        if array.ndim == sample_ndim + 1:
            return array[:, None, ...]
        if array.ndim == sample_ndim + 2:
            return array
        raise ValueError(f"Unsupported GR00T N1.7 observation shape: {array.shape}")

    @staticmethod
    def _strip_batch_axis(array: np.ndarray) -> np.ndarray:
        if array.ndim == 3:
            return array[0]
        if array.ndim == 2:
            return array
        if array.ndim == 1:
            return array[None, :]
        raise ValueError(f"Unsupported GR00T N1.7 action shape: {array.shape}")

    def get_action(self, observation_dict: dict) -> torch.Tensor:
        obs_dict = {
            "video": {
                key: self._add_batch_time_axes(observation_dict[key].cpu().numpy().astype(np.uint8), sample_ndim=3)
        ...
        if "single_arm" in self.modality_keys:
            joint_pos = convert_leisaac_action_to_lerobot(observation_dict["joint_pos"]).astype(np.float32)
            obs_dict["state"]["single_arm"] = self._add_batch_time_axes(joint_pos[:, 0:5], sample_ndim=1)
            obs_dict["state"]["gripper"] = self._add_batch_time_axes(joint_pos[:, 5:6], sample_ndim=1)
        # TODO: add bi-arm support
        ...
        if not isinstance(response, (list, tuple)) or len(response) != 2:
            raise RuntimeError(f"Unexpected GR00T N1.7 response type: {type(response).__name__}")

        action_chunk, _info = response
        if "action" in action_chunk:
            action_chunk = action_chunk["action"]

        single_arm_action = action_chunk.get("single_arm", action_chunk.get("action.single_arm"))
        gripper_action = action_chunk.get("gripper", action_chunk.get("action.gripper"))
        if single_arm_action is None or gripper_action is None:
            raise RuntimeError(f"Unexpected GR00T N1.7 action keys: {list(action_chunk.keys())}")

        concat_action = np.concatenate(
            [
                self._strip_batch_axis(np.asarray(single_arm_action)),
                self._strip_batch_axis(np.asarray(gripper_action)),
            ],
            axis=1,
        )
        concat_action = convert_lerobot_action_to_leisaac(concat_action)
