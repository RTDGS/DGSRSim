...
parser.add_argument(
    ...
    type=str,
    default="gr00tn1.5",
    help="Type of policy to use. support gr00tn1.5, gr00tn1.7, lerobot-<model_type>, openpi",
)
...
parser.add_argument("--record", action="store_true", help="Record successful inference episodes to an HDF5 dataset.")
parser.add_argument(
    "--dataset_file",
    type=str,
    default="./datasets/policy_inference_dataset.hdf5",
    help="File path to export successful recorded inference episodes.",
)
...
def preprocess_obs_dict(obs_dict: dict, model_type: str, language_instruction: str):
    ...
    if model_type in ["gr00tn1.5", "gr00tn1.7", "lerobot", "openpi"]:
...
def setup_recording_cfg(env_cfg, dataset_file: str):
    """Configure recorder to keep only successful episodes."""
    if env_cfg.recorders is None:
        raise RuntimeError("This environment does not define a recorder configuration.")

    output_dir = os.path.dirname(dataset_file) or "."
    output_file_name = os.path.splitext(os.path.basename(dataset_file))[0]
    if not output_file_name:
        raise ValueError(f"Invalid dataset file path: {dataset_file}")

    normalized_dataset_file = os.path.join(output_dir, output_file_name + ".hdf5")
    if os.path.exists(normalized_dataset_file):
        raise FileExistsError(f"Dataset file already exists: {normalized_dataset_file}")
...
def main():
    ...
    if args_cli.record:
        setup_recording_cfg(env_cfg, args_cli.dataset_file)
    else:
    ...
    if args_cli.policy_type in ["gr00tn1.5", "gr00tn1.7"]:
        from isaaclab.sensors import Camera
        from leisaac.policy import Gr00tN17ServicePolicyClient, Gr00tServicePolicyClient

        if task_type == "so101leader":
            modality_keys = ["single_arm", "gripper"]
        else:
            raise ValueError(f"Task type {task_type} not supported when using GR00T policy yet.")

        policy_client_cls = (
            Gr00tN17ServicePolicyClient if args_cli.policy_type == "gr00tn1.7" else Gr00tServicePolicyClient
        )

        policy = policy_client_cls(
