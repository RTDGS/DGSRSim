...
def get_prim_world_aabb_size(stage, prim_path: str):
    """
    Return world-space AABB size (dx, dy, dz) in meters for prim_path.
    Compatible with older pxr bindings where BBoxCache ctor does NOT accept kwargs.
    """
    ...
    # IMPORTANT: older Boost.Python bindings require positional args:
    # BBoxCache(time, includedPurposes [, useExtentsHint [, ignoreVisibility]])
    ...
    try:
        ...
        # fallback: some builds may only support 2-arg ctor
...
def create_static_collider_box(
    ...
    """
    Static collider:
    - Only CollisionAPI on a cube geom is enough (no RigidBody => static in PhysX).
    """
    import omni.usd
...
def get_world_xf(stage, prim_path: str):
    """Get prim world transform as 4x4 numpy (row-major)."""
    import numpy as np
    ...
    M = cache.GetLocalToWorldTransform(prim)  # Gf.Matrix4d

    # Gf.Matrix4d -> numpy
    # 注意：Gf.Matrix4d 的 GetRow(i) 最稳，避免 GetArray 顺序差异
...
def set_prim_world_matrix(stage, prim_path: str, T_world):
    """Overwrite prim xform with one TransformOp using a world matrix."""
    from pxr import UsdGeom, Gf
    import numpy as np
...
def make_rigidbody_kinematic(stage, prim_path: str):
    ...
    # 兼容不同版本属性
    for fn in ["CreateKinematicEnabledAttr", "GetKinematicEnabledAttr"]:
        if hasattr(physx_rb, fn):
            try:
                attr = getattr(physx_rb, fn)()
                # CreateXxxAttr 返回 attr；GetXxxAttr 返回 attr
                if attr is not None:
                    attr.Set(True)
            except Exception:
                pass
...
def main():  # noqa: C901
    ...
    # preprocess timeouts/terminations
    ...
    # Key bindings:
    # R: reset + stop recording (hard stop)
    # O: toggle recording (resume/start)
    ...
    # -----------------------------
    # Spawn RuntimeMug_proxy (dynamic) + visual
    #   - AUTO scale visual to TARGET_VISUAL_SIZE_M
    # -----------------------------
    import omni.usd

    stage = omni.usd.get_context().get_stage()

    TARGET_VISUAL_SIZE_M = (0.08, 0.08, 0.12)  # (x,y,z) in meters for the final 2.usdz visual size
    AUTO_FIT_AXIS = "z"  # recommended for mugs: match height; options: "x","y","z","max"
    ...
    for i in range(env.num_envs):
        ...
        # proxy collider size should roughly match the target
