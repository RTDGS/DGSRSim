...
# NEW: absolute pose npy (template/mug -> Scene)
parser.add_argument(
    "--pose_npy",
    type=str,
...
def make_rigidbody_kinematic(stage, prim_path: str, disable_gravity: bool = True):
    ...
    # Try to set kinematic enabled across versions
...
def main():  # noqa: C901
    ...
    for i in range(env.num_envs):
        ...
        # NEW: set kinematic (absolute pose driven)
        ...
        while simulation_app.is_running():
            with torch.inference_mode():
                ...
                if should_reset:
                    ...
                    # -----------------------------
                    # NEW: apply absolute pose to RuntimeMug_proxy each step
                    # T_scene_mug = (mug/template -> scene)
                    # T_world_mug = T_world_scene @ T_scene_mug
                    # -----------------------------
