...
- In rule-grasp mode, teleop_interface is _NoUI so it does NOT receive keyboard events.
  This script installs a Kit keyboard listener so you can press pose_sync_key (default K)
  to toggle pose sync at runtime, with manual override priority over agent requests.
"""
...
parser.add_argument(
    ...
    type=str,
    default="M",
    help="Key to toggle pose synchronization ON/OFF (default: M).",
)
parser.add_argument(
    "--pose_sync_freeze",
    action="store_true",
...
class RuleGraspAgent:
    ...
    def _get_ee_world_T_best_effort(self):
        ...
        raise RuntimeError(
            ...
            "(e.g. 'Robot/gripper' or 'Robot/jaw') or adapt _get_ee_world_T_best_effort()."
        )
...
def main():  # noqa: C901
    ...
    # pose sync flags
    pose_sync_enabled = True  # initial ON
    ...
    # ------------------------------------------------------------
    # Manual pose-sync override for rule-grasp (Kit keyboard listener)
    #   manual_pose_sync_override:
    #     - None: follow agent requests
    #     - True/False: force pose_sync_enabled state
    # ------------------------------------------------------------
    manual_pose_sync_override = None
    _kit_input = None
    _kit_keyboard = None
    _kit_sub = None

    def _kit_key_to_input(key_name: str):
        import carb
        key_name = str(key_name).upper()
        mapping = {
            "A": carb.input.KeyboardInput.A,
            "B": carb.input.KeyboardInput.B,
            "C": carb.input.KeyboardInput.C,
            "D": carb.input.KeyboardInput.D,
            "E": carb.input.KeyboardInput.E,
            "F": carb.input.KeyboardInput.F,
            "G": carb.input.KeyboardInput.G,
            "H": carb.input.KeyboardInput.H,
            "I": carb.input.KeyboardInput.I,
            "J": carb.input.KeyboardInput.J,
            "K": carb.input.KeyboardInput.K,
            "L": carb.input.KeyboardInput.L,
            "M": carb.input.KeyboardInput.M,
            "N": carb.input.KeyboardInput.N,
            "O": carb.input.KeyboardInput.O,
            "P": carb.input.KeyboardInput.P,
            "Q": carb.input.KeyboardInput.Q,
            "R": carb.input.KeyboardInput.R,
            "S": carb.input.KeyboardInput.S,
            "T": carb.input.KeyboardInput.T,
            "U": carb.input.KeyboardInput.U,
            "V": carb.input.KeyboardInput.V,
            "W": carb.input.KeyboardInput.W,
            "X": carb.input.KeyboardInput.X,
            "Y": carb.input.KeyboardInput.Y,
            "Z": carb.input.KeyboardInput.Z,
            "SPACE": carb.input.KeyboardInput.SPACE,
        }
        return mapping.get(key_name, None)

    def _install_rule_grasp_key_listener():
        """
        More robust keyboard listener across Isaac Sim / Kit versions.

        Key points:
        - Prefer omni.kit.app.get_app().get_platform_keyboard() for a valid keyboard device
        - Prefer subscribe_to_keyboard_events; fallback to subscribe_to_input_events
        - Add debug probe so you can confirm whether events are arriving at all
        """
        nonlocal _kit_input, _kit_keyboard, _kit_sub, manual_pose_sync_override, pose_sync_enabled
        import carb
        import omni.kit.app

        key_input = _kit_key_to_input(args_cli.pose_sync_key)
        if key_input is None:
            print(f"[pose-sync] WARN: unsupported pose_sync_key='{args_cli.pose_sync_key}'. Use A-Z or SPACE.")
            return

        _kit_input = carb.input.acquire_input_interface()

        # ---- obtain platform keyboard (MOST stable across versions)
        ...
        # ---- fallback: ask input interface (less stable)
        if kb is None:
            for name in ["get_keyboard_device", "get_keyboard", "get_default_keyboard"]:
                if hasattr(_kit_input, name):
                    try:
                        kb = getattr(_kit_input, name)()
                        if kb is not None:
                            break
                    except Exception:
                        kb = None
        ...
        # ---- manual toggle
        def _toggle_pose_sync_manual():
            nonlocal manual_pose_sync_override, pose_sync_enabled
            # Toggle manual override priority over agent
            if manual_pose_sync_override is None:
                manual_pose_sync_override = (not pose_sync_enabled)
            else:
                manual_pose_sync_override = (not manual_pose_sync_override)

            pose_sync_enabled = bool(manual_pose_sync_override)
            print(
                f"[pose-sync][MANUAL] {'ON' if pose_sync_enabled else 'OFF'} "
                f"(override={'ON' if manual_pose_sync_override else 'OFF'}) "
                f"(freeze_off={freeze_when_sync_off})"
            )

        # ---- DEBUG probe (prints first few events)
        _probe = {"n": 0, "max": 30}

        def _probe_print(tag: str, event):
            if _probe["n"] >= _probe["max"]:
                return
            _probe["n"] += 1
            try:
                t = getattr(event, "type", None)
                inp = getattr(event, "input", None)
                dt = getattr(event, "deviceType", None)
                print(f"[pose-sync][probe:{tag}] type={t} input={inp} deviceType={dt}")
            except Exception:
                print(f"[pose-sync][probe:{tag}] event={event}")

        # ---- keyboard-events callback
        def _on_keyboard_event(event, *args, **kwargs):
            _probe_print("kb", event)

            # Press filter (different versions may expose different enums)
            try:
                if hasattr(carb.input, "KeyboardEventType") and hasattr(event, "type"):
            ...
            # Key match
            try:
                if hasattr(event, "input") and event.input == key_input:
                    _toggle_pose_sync_manual()
            except Exception:
                pass

        # ---- input-events callback (fallback)
        def _on_input_event(event, *args, **kwargs):
            import carb

            # 只处理键盘
            try:
                if hasattr(event, "deviceType") and hasattr(carb.input, "DeviceType"):
            ...
            # 常见：value=1 表示 press，0 表示 release；有时是 float
            ...
            # 有些版本用 action/state/type（但你这里 type=None）
            for name in ["action", "state", "eventType"]:
                if pressed is None and hasattr(event, name):
                    try:
                        v = getattr(event, name)
                        # 只要是“按下/press/down”语义就算 pressed
                        if isinstance(v, str):
                            pressed = ("press" in v.lower()) or ("down" in v.lower())
                        else:
                            # 枚举：尽量转字符串判断
                            pressed = ("press" in str(v).lower()) or ("down" in str(v).lower())
                    except Exception:
                        pass
            ...
            # --------- 硬验证：任何键按下都切换 ---------
            print("[pose-sync] DEBUG: got KEYBOARD press event (any key).")
            _toggle_pose_sync_manual()

        # ---- subscribe
        if kb is not None and hasattr(_kit_input, "subscribe_to_keyboard_events"):
            try:
                _kit_keyboard = kb
                _kit_sub = _kit_input.subscribe_to_keyboard_events(_kit_keyboard, _on_keyboard_event)
                print(f"[pose-sync] Kit key listener active: '{str(args_cli.pose_sync_key).upper()}' (keyboard-events)")
                print("[pose-sync] NOTE: click the Isaac Sim viewport once to ensure it has focus, then press the key.")
                return
            except Exception as e:
                print(f"[pose-sync] WARN: subscribe_to_keyboard_events failed: {e}")

        if hasattr(_kit_input, "subscribe_to_input_events"):
            try:
                _kit_keyboard = None
                _kit_sub = _kit_input.subscribe_to_input_events(_on_input_event)
                print(
                    f"[pose-sync] Kit key listener active: '{str(args_cli.pose_sync_key).upper()}' (input-events fallback)")
                print("[pose-sync] NOTE: click the Isaac Sim viewport once to ensure it has focus, then press the key.")
                return
            except Exception as e:
                print(f"[pose-sync] ERROR: subscribe_to_input_events failed: {e}")

        print("[pose-sync] ERROR: No compatible input subscription API found in this Kit build.")

    if args_cli.teleop_device == "rule-grasp":
        _install_rule_grasp_key_listener()
    ...
    # callbacks (no-op for rule-grasp because teleop_interface=_NoUI)
    ...
    print(f"[pose-sync] initial=ON. Manual toggle key='{str(args_cli.pose_sync_key).upper()}' (rule-grasp uses Kit listener).")
    if args_cli.teleop_device == "rule-grasp":
        print("[pose-sync] NOTE: manual override has priority over RuleGraspAgent requests.")
    ...
    try:
        while simulation_app.is_running():
            with torch.inference_mode():
                ...
                if args_cli.teleop_device == "rule-grasp":
                    ...
                    # Agent requests apply ONLY when there is no manual override
                    if manual_pose_sync_override is None:
                ...
                if should_reset:
                    ...
                    if pose_sync_enabled:
                        ...
                        if freeze_when_sync_off:
                            ...
                            # OFF + no freeze: do not overwrite RuntimeMug_proxy pose
                            pass
        ...
        # cleanup kit keyboard listener
        # cleanup kit keyboard listener
        try:
            if _kit_input is not None and _kit_sub is not None:
                if _kit_keyboard is not None:
                    for name in ["unsubscribe_to_keyboard_events", "unsubscribe_from_keyboard_events"]:
                        if hasattr(_kit_input, name):
                            getattr(_kit_input, name)(_kit_keyboard, _kit_sub)
                            break
                else:
                    for name in ["unsubscribe_to_input_events", "unsubscribe_from_input_events"]:
                        if hasattr(_kit_input, name):
                            getattr(_kit_input, name)(_kit_sub)
                            break
        except Exception:
            pass
