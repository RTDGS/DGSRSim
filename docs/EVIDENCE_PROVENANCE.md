...
- The aggregate values used by the manuscript's supplementary Fig. 8 in `figure8_table_derived_diagnostic.csv`.
...
- Raw per-frame object-load and runtime logs behind Tables 6 and 7.
...
The CSV contains 12 table-derived rows: eight PSNR entries copied from manuscript Table 6 and four cumulative stage means derived from the stage entries in Table 7. It is a plotting record for a supplementary diagnostic, not a raw per-object, per-frame benchmark.
