...
def rot_to_yaw_pitch_roll(R: np.ndarray) -> Tuple[float, float, float]:
    """
    Yaw-Pitch-Roll (Z-Y-X intrinsic) commonly used.
    Returns (yaw, pitch, roll).
    """
    R = np.asarray(R, dtype=np.float64).reshape(3, 3)
    # pitch = asin(-R31) for ZYX
    sy = -R[2, 0]
...
def align_and_build_pointcloud_strict_with_pixcolor(
    ...
    """
    说明：
    - Kinect CameraSpace: x,y,z (meters)
    - 你之前 flip_yz_for_o3d=True 会把 y,z 都取负，常见表现就是“整体倒过来/翻转”。
    - 这里拆成 flip_y / flip_z，默认都 False（先保证不倒）。
    """
    if depth_u16.dtype != np.uint16 or depth_u16.ndim != 2:
        raise ValueError(f"depth_u16 must be uint16 (Hd,Wd). Got {depth_u16.dtype}, {depth_u16.shape}")

    depth_h, depth_w = depth_u16.shape
    ...
    cam_np = np.ctypeslib.as_array(cam_points)
    col_np = np.ctypeslib.as_array(col_points)

    # xyz
    if cam_np.dtype.names:
        x = cam_np["x"].astype(np.float32, copy=False)
        y = cam_np["y"].astype(np.float32, copy=False)
        z = cam_np["z"].astype(np.float32, copy=False)
    else:
        cam_np2 = cam_np.view(np.float32).reshape(-1, 3)
        x, y, z = cam_np2[:, 0], cam_np2[:, 1], cam_np2[:, 2]

    # uv
    if col_np.dtype.names:
        u = col_np["x"].astype(np.float32, copy=False)
        v = col_np["y"].astype(np.float32, copy=False)
    else:
        col_np2 = col_np.view(np.float32).reshape(-1, 2)
        u, v = col_np2[:, 0], col_np2[:, 1]
...
class AsyncRegistrar:
    """
    Background registration; run only when trigger_once() called.
    关键：T 是 T_src_to_tgt（src: scene object, tgt: template）
    你要的“模板在原场景的位姿”是 inv(T)。
    """
    ...
    def _register_once(self, src_used: o3d.geometry.PointCloud) -> Tuple[
        np.ndarray, Dict[str, Any], o3d.geometry.PointCloud]:
    ...
    def _loop(self):
        while self._running:
            ...
            try:
                ...
                src_aligned.transform(T)  # scene -> template
    ...
    def run_once(self) -> bool:
        ...
        if self.strict_ok and now >= self._pcd_next_t:
            ...
            try:
                ...
                # cache src for registrar (only cache; do not run automatically)
        ...
        if metrics.get("ok", False) and T_src_to_tgt is not None:
            ...
            # --------- NEW: pose in original scene (template placed into scene) ----------
            # T_src_to_tgt maps: scene_object(src) -> template(tgt)
            # We want: template(tgt) -> scene frame, so invert it.
            T_tgt_to_scene = np.linalg.inv(np.asarray(T_src_to_tgt, dtype=np.float64))

            # avoid printing every frame: print when mts changes (i.e., new src timestamp)
...
if __name__ == "__main__":
    ...
    # 如果你发现是“前后”反了：把 flip_z=True 试试；
    # 不要再默认同时 flip_y 和 flip_z。
    FLIP_Y = False
    FLIP_Z = False
